...
```
  ...
  發件箱  ─────▶│  email_source        │  IMAP 或 本地 .mbox/.eml
                ...
                └──────────┬───────────┘
                           │
  額外輸入 ───────────────▶│  report_builder  依日期整理；郵件分別列出、
 (姓名/部門/每日工作/其他)   │                   並合併每日自行輸入的工作內容
                           ▼
                ┌──────────────────────┐
...
- **郵件記錄**：把當天發件箱的**每一封郵件分別列出**成一條工作內容（會自動去除 `Re:`／`Fwd:` 等前綴）。若想把同主題對話合併成一條，可加 `--merge-threads`。
- **自行輸入**：每日的日常工作內容提供**輸入接口**，可由你自行補充。三種方式：
  - 命令列 `--daily YYYY-MM-DD=內容`（多項用「；」分隔，可重複）
  - 互動模式 `--interactive`（逐日詢問）
  - 設定檔 `manual_items`，或程式 API `ReportConfig.add_daily_work(day, text)`

「其他事項」同樣可由設定檔或命令列 `--other` 提供。
...
python -m weekly_report \
  ...
  --daily "2026-06-04=撰寫設計文件；與供應商開會" \
  --other "下週準備 Q2 簡報" --other "6/12 請假一天" \
  -f text
```

> `--daily` 是每日日常工作內容的輸入接口：當天郵件會自動分別列出，
> 你再用 `--daily`（或 `--interactive` 逐日輸入）補充其他工作內容即可。
...
| `--daily 日期=內容` | 每日日常工作內容的輸入接口；多項用「；」分隔，可重複 |
| `--merge-threads` | 把同一天同主題的郵件合併成一條（預設每封分別列出） |
| `--other 事項` | 其他事項，可重複多次 |
| `--interactive` | 互動模式，逐日詢問日常工作內容並補充其他事項 |
