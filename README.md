...
| `run_dir` | `output_dir` 留空 → 與 `vba_dir` 相同；有填 → `output_dir\yyyymmdd\` | `LME每日報價yyyymmdd.xlsx`、當日 `lme_daily.log` |
...
**不要**把 QueryTable 的 `TEXT;P:\...\LME SPAN\lme.yyyymmdd.dat` 改成 UNC。Excel 的 TEXT 連線對 `\\server\share` 常直接 1004「找不到檔案」，即使 Python / 檔案總管都看得到同一個 `.dat`。上一版在跑巨集前把 P: 改成 UNC，就是修改後找不到 dat 的原因。現在預設會把誤改的 UNC **改回 P:**（記憶體、不存檔）。`paths.span_dat_dir` 留空則檢查 `working_dir` 同一層的 `LME SPAN\lme.{上日}.dat`。請把 `examples/TestPDriveVisible.bas` 匯入參考工作簿；**不要**按 `ReplacePDriveWithUnc.bas` 去永久取代 P: 路徑。
...
4. **合併版面** — 把上述三塊排在同一橫向 A3 版面（金棕 `B6895F` 標題列、宋体、中英雙語標題、頁碼「第 n 頁，共 N 頁」），底部為完整兩段免責聲明。不匯出 PDF。樣式常數集中在 `report_builder.MergedLayoutStyle`。第 2 列（合併 A2:H2）用 openpyxl `add_image` 嵌入 `branding.logo_path` 的浮動圖片（高度對齊列高 34、寬度等比縮放，並以 `center_image_in_merged_range` 水平／垂直置中）；未設定或檔案不存在只記 WARNING，不中斷。不要用 Excel 365 的 `DISPIMG` 公式。免責聲明列號依最後一筆內容動態計算（`last_content_row + 2`），A:H 欄寬一律 `apply_autofit_columns`。
