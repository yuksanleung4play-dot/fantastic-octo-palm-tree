...
PRINT_CHART_BUFFER_ROWS = 2  # 相鄰兩組走勢圖之間預留列數，避免邊界重疊
PRINT_CHART_COL_POSITIONS = (0, 3)  # 0-based：左圖 A 欄、右圖 D 欄
PRINT_CHART_NATIVE_WIDTH_PX = round(6.4 * 120)  # matplotlib figsize × dpi
...
PRINT_FOOTER_SIZE = 9
PRINT_FOOTER_HF_SIZE = "09"  # Excel ``&nn`` 兩位字級，避免與 ``&P`` 粘連
...
class MergedLayoutStyle:
    ...
    def chart_width_px(cls) -> int:
        return print_chart_display_pixel_size()[0]

    @classmethod
    def chart_height_px(cls) -> int:
        return print_chart_display_pixel_size()[1]
...
def print_chart_display_pixel_size(
    *,
    row_gap: int = PRINT_CHART_ROW_STRIDE,
    buffer_rows: int = PRINT_CHART_BUFFER_ROWS,
    row_height_pt: float = float(DEFAULT_ROW_HEIGHT),
    design_width_emu: int = PRINT_CHART_WIDTH_EMU,
    design_height_emu: int = PRINT_CHART_HEIGHT_EMU,
) -> tuple[int, int]:
    """走勢圖插入尺寸：高等於可用列高（間隔列 − 緩衝列），寬度等比例縮小。"""
    usable_rows = max(1, int(row_gap) - int(buffer_rows))
    max_h = max(1, int(round(points_to_pixels(row_height_pt * usable_rows))))
    design_w = design_width_emu / EMU_PER_PIXEL
    design_h = design_height_emu / EMU_PER_PIXEL
    scale = min(1.0, max_h / design_h) if design_h else 1.0
    return max(1, int(round(design_w * scale))), max(1, int(round(design_h * scale)))


def chart_pair_page_break_rows(
    start_row: int, n_pairs: int, row_gap: int
) -> list[int]:
    """1-based：在每組走勢圖之前／之間／之後插入列分頁，避免一張圖被切成兩頁。"""
    rows: list[int] = []
...
def _chart_slot_usable_height_px(
    ws: Worksheet,
    start_row: int,
    row_gap: int,
    buffer_rows: int,
    ...
    usable_rows = max(1, int(row_gap) - int(buffer_rows))
    total_pt = 0.0
    for offset in range(usable_rows):
        total_pt += worksheet_row_height_points(ws, start_row + offset)
    return max(1, int(round(points_to_pixels(total_pt))))
...
def place_forward_curve_images(
    ...
    start_row: int,
    row_gap: int,
    col_positions: tuple[int, ...] = PRINT_CHART_COL_POSITIONS,
    *,
    buffer_rows: int = PRINT_CHART_BUFFER_ROWS,
) -> int:
    """合併版面六張走勢圖：2 欄 × 3 列。高度小於列距並預留緩衝，避免上下重疊。"""
    n_cols = max(1, len(col_positions))
    if not images:
        return start_row
    n_pairs = (len(images) + n_cols - 1) // n_cols
    slot_h = _chart_slot_usable_height_px(ws, start_row, row_gap, buffer_rows)
    design_w = PRINT_CHART_WIDTH_EMU / EMU_PER_PIXEL
    design_h = PRINT_CHART_HEIGHT_EMU / EMU_PER_PIXEL
    scale = min(1.0, slot_h / design_h) if design_h else 1.0
...
def _place_forward_curve_images_xlsxwriter(
    ...
    *,
    ...
    """xlsxwriter 版 ``place_forward_curve_images``。``start_row`` 為 Excel 1-based 列號。"""
    n_cols = max(1, len(col_positions))
    if not images:
        return start_row
    n_pairs = (len(images) + n_cols - 1) // n_cols
    width, height = print_chart_display_pixel_size(
        row_gap=row_gap, buffer_rows=buffer_rows
    )
...
def _place_print_chart_images(ws: Worksheet, images: list[Path], *, start_row: int) -> int:
    ...
    return place_forward_curve_images(
        ...
        images,
        start_row,
        MergedLayoutStyle.chart_row_stride,
...
def _write_print_sheet_xlsxwriter(
    ...
    *,
    ...
    after_images_1 = _place_forward_curve_images_xlsxwriter(
        ws,
        chart_images,
